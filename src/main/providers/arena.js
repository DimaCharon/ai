...
class Arena extends Provider {
  ...
  async check() {
    ...
    if (r.status === 401) {
      throw new ProviderError(
        "session_expired",
        "Сессия Arena AI недействительна (HTTP 401). Важно: ACCESS-ТОКЕН АРЕНЫ ЖИВЁТ ~1 ЧАС — " +
        "куку периодически (раз в час-два) нужно копировать заново. Открой arena.ai в браузере, " +
        "убедись, что ты вошёл, скопируй куку заново (F12 → Network → Request Headers → cookie, " +
        "вставь ПРЯМО в приложение, не через чат — длинные строки искажаются при пересылке) " +
